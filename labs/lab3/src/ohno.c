...
ohno(const char *message, ohno_severity_t severity)
{
	...
	{
		fprintf(state->out, "WARNING: %s Name: %s\n", message, state->name);
	}
	if(severity == OHNO_SERIOUS)
	{
		fprintf(state->out, "SERIOUS: %s Name: %s\n", message, state->name);
	}
	if(severity == OHNO_FATAL)
	{
		fprintf(state->out, "FATAL-ERROR: %s Name: %s\n", message, state->name);
	}
}
