...
The goal of this lab is to use C++ classes and inheritance to create simple letter filters.

Instructions:

(1) Read this file and understand the basic operations and interactions between these objects.
...
(5) Modify `getFilter` to test your Encrypt class.
      Note: if you want to verify your Encrypt filter, instantiate with a shift of 13 and pipe a message through the filter twice.
...
class Filter
{
  ...
  // It is `virtual` so the compiler knows to always call the overriden version in our more interesting filters.
  // The base class's implementation here just passes each character unchanged.
  //
...
string Filter::Exec(string input)
{
  ...
  for (int i = 0; i < input.size(); i++) {
    ...
    if (isalpha(input[i])) {
      ...
      // Note that the call to FilterLetter will execute the implementation of FilterLetter for the class of the object on which this method is invoked.
      // This allows us to use this same code for each filter which inherits from the base Filter class.
...
//
// An example of inheriting from the Filter base class to implement a filter which makes each alphabetic character uppercase
//
...
// Each character is converted to uppercase then shifted a fixed number of letters.
//
char
...
//
// TODO (2) Define an Encrypt filter which uses the above shift_cypher() function to encode its input letters.
